...
- `round_done` also now carries `lives` (this client's remaining lives after the round) and `round` (1-based round number just finished). The server relays these as `{ type: 'peer_progress', clientId, lives, round, finished }` to every other player in the room, purely so their opponents panel can show your lives/round -- it isn't used for any actual round-advance logic.
